...
乱序处理器是当今处理器使用的方案,它采用一种投机的方式,尽可能多的把指令进行流水线化,即使遇到分支,也会先进行预测,预测的指令会直接进入流水线,预测错误的分支指令会丢弃,会有性能损失.

同时处理器的不同模块会有很多相同的功能单元,比如加法的单元有多份,来支持并行的计算,访存的单元也会有多份.

底层会把一些复杂的执行,分解成多条小指令,来让流水线(多硬件)执行效率更高.这也吸取了一些精简指令集的经验,怎么转换的,这些都是商业绝密.

![单元硬件](../Source/pic/csapp/hardware.png)
从途中可以看到,有几个单元都有加载  有几个都有 浮点乘等等.

### 功能单元的性能
![单元硬件](../Source/pic/csapp/5-12.png)
有几个时间:
* 延迟时间: 实际运算需要的周期数
* 发射时间: 需要等多少个周期才能发送相同的指令
* 容量:有多少个能处理这个的硬件数目

可以看出,加法执行很快,乘法也能隔一个周期往流水线送一条指令.除法的延迟时间和发射时间几乎是相同的,执行很慢,往往需要一个除法执行完了才能执行下一个,同时只有一个除法的单元,可能这和除法很少用到有关.

设计处理器,就是那些常用操作给多点硬件,让延迟小点,多点指令今流水线,而那些不常用的除法就不需要那多硬件,延迟高点也没啥.

这些硬件上的延迟时间和发射时间以及容量,就会直接决定了程序运行时间的下界,换句话来说,他们直接决定了理想状态下最优的运行时间,所有的优化都最终会接近这个下界,而无法等于下界.

## K*K次循环展开
* K次循环展开:这里主要介绍的是,循环每次处理一个数据,如果改成每次处理2个,就会将循环的次数降低一半,此时循环带来的额外损耗就会降低,优化后,CPE降低.
* K*K次循环展开:
 我们把K*K次循环展开称为"将循环展开K次,并累计K个值"
 我们发现,K次循环展开后,循环依赖变量限制了程序不能运行的更快,我们把循环依赖变量转用K个变量表示,在循环外再合并K个变量,这样就能将一次循环内部的K次操作进行并行处理,用不同的寄存器,不相互依赖,将会大大提升CPE.

**关于寄存器溢出**
我们可以无限的展开来提升CPE吗?对比10*10和20*20的CPE,20*20有时还会相对慢,这是因为,太多的循环展开,寄存器无法保存太多的变量,会保存在栈上,就需要访存,此时降低了效率,所以,循环展开并不是无限的,和寄存器的数量是有关系的.

## 重新结合变换
比如一个循环中有num1 = num1 * (*sr) * (*pp), 此时可以把后两项结合,效率更快,这是因为,后两项是两个访存,先操作可以并行提升效率,而前半部分的num1值依赖上次循环的num1会等待,导致执行慢.
此方法对浮点数不适用,因为浮点数不满足结合律,因为不同的舍入结合不同结果不同.

这一步操作的另一个依赖是,处理器加载内存的硬件是多份的,这样可以让加载并行操作.
对于num1 = num1 *...,这种num1被称为`循环依赖变量`,也就是这个变量依赖上次循环的结果,如果可以把这个元素的操作后执行将会提升系统的并行效率.
